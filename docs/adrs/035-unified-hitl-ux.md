...
2. Resolves the calling pod's instance ID from two independent sources that must agree:
   ...
   - **`x-humr-instance` initial_metadata** (defense-in-depth). Envoy renders it into every Check call from the per-instance bootstrap. The header is required and must equal the IP-derived instance ID. Mismatch = forged header or bootstrap drift; missing = someone bypassed Envoy. Both fail closed.
   A compromised agent that bypasses its sidecar can't satisfy either gate alone: it can't spoof the source IP, and even if it could, it'd need to know the right `x-humr-instance` for that IP — and the only winning header is its own instance's, which gives it the identity it already has.
