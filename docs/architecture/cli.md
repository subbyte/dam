...
- `dam connection connect <app-id>` — create a Connection from a provider template. Resolves the template by id (unknown id or an MCP-category template exits `EXIT_INVALID_INPUT`), collects the inputs the template declares (typed flags such as `--client-id`/`--value`, with interactive `@clack` prompts filling any *required* gap on a TTY — secrets masked — and a non-TTY gap erroring out naming the missing flag), then `connections.create`. For an OAuth template it `connections.startOAuth` → opens the system browser (`--no-browser` prints the URL instead; an open failure degrades to printing) → polls `connections.get` every 2s up to `--timeout` (default 300s, under the server's 10-min pending-flow TTL) until `status === "active"`. Header/none templates need no browser and report success immediately. A poll timeout exits non-zero with a "check `dam connection list`" hint and leaves the connection in place (it may complete late). `--json` emits `{ ok, id, status, authKind }` (plus `authUrl` under `--no-browser`).
...
Everything is tRPC (`connections.list`, `listTemplates`, `create`, `startOAuth`, `get`, `getAgentConnections`, `setAgentConnections`, `delete`) — there is no REST surface and no service state beyond the per-host client. Three pieces of non-obvious behavior:
...
`connect` does not support MCP-server templates yet (the same follow-up #344 deferred). Listing, granting, revoking, and disconnecting are unchanged.
