...
import { useApprovalsForAgent } from "../api/queries.js";
import { ApprovalsList } from "./approvals-list.js";

const EMPTY: never[] = [];

export interface AgentApprovalsTrayProps {
  agentId: string | null;
}
 ...
 */
export function AgentApprovalsTray({ agentId }: AgentApprovalsTrayProps) {
  const { data: rows = EMPTY } = useApprovalsForAgent(agentId);
