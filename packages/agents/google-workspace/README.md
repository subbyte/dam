...
A Platform agent template with the [Google Workspace CLI (`gws`)](https://github.com/googleworkspace/cli) pre-installed for managing Google Drive, Gmail, Calendar, Sheets, and more.
...
4. **Configure OAuth consent screen** — go to **APIs & Services > OAuth consent screen**:
   ...
   - App name: anything (e.g., "Platform Agent")
...
1. Open the platform UI at http://localhost:4444
...
The agent authenticates to Google APIs through the platform's credential injection ([ADR-033](../../../docs/adrs/033-envoy-credential-gateway.md)):

1. When you grant a `gmail` or `google-drive` connection in the agent's Configure dialog, the platform auto-populates `GOOGLE_WORKSPACE_CLI_TOKEN=dummy-placeholder` into the agent's editable env list. You can edit or remove it like any custom env var.
2. When `gws` makes a request to `*.googleapis.com`, it sends `Authorization: Bearer dummy-placeholder`.
...
The platform stores the OAuth refresh token in a K8s Secret and runs a refresh loop in the api-server that re-mints the access token before it expires. No manual intervention is needed after the initial OAuth consent.
