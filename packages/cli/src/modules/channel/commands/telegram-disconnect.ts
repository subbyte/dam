...
export function buildTelegramDisconnectCommand(deps: {
  ...
  return new Command("disconnect")
    .description(
      "Unbind an Agent's Telegram bot (deletes its k8s Secret). Idempotent — a not-connected agent exits 0.",
    )
    ...
    .addHelpText(
      "after",
      "\nExamples:\n  dam channel telegram disconnect my-agent\n",
    )
    .action(async (ref: string, opts: { server?: string; json?: boolean }) => {
      ...
      const res = await svc.disconnectTelegram(resolved.value.id);
      if (!res.ok) {
      ...
      if (opts.json) {
        ...
        process.stdout.write(
          `✓ Telegram disconnected from ${resolved.value.name}.\n`,
        );
      }
