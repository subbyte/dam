...
import type { MalformedConfigError, MissingConfigError, ProbeError } from "../domain/errors.js";
...
export function buildPingCommand(deps: { service: CompatService; configService: ConfigService }): Command {
  return new Command("ping")
    ...
    .action(async (opts: { server?: string }) => {
      ...
      if (!cfg.ok) { printResolveError(cfg.error, ""); process.exit(EXIT_RUNTIME_FAILURE); }
      const host = cfg.value.server;

      const result = await deps.service.check({ flag });
      if (!result.ok) { printResolveError(result.error, host); process.exit(EXIT_RUNTIME_FAILURE); }
      ...
      switch (verdict.kind) {
        case "ok":
          ...
          return;
        case "behind-current":
          process.stderr.write(`warning: CLI ${verdict.localCli} is behind server ${verdict.serverVersion}; consider upgrading\n`);
          process.stdout.write(`ok — server ${verdict.serverVersion}\n`);
          return;
        case "below-floor":
          process.stderr.write(`error: CLI ${verdict.localCli} is below the server's minimum required version ${verdict.serverMinClient}; upgrade and retry\n`);
          process.exit(EXIT_COMPAT_BELOW_FLOOR);
      }
    });
}

function printResolveError(e: MissingConfigError | MalformedConfigError | ProbeError, host: string): void {
  switch (e.kind) {
    case "missing-config":
    ...
    case "malformed-config":
      ...
      return;
    case "probe-error": {
      const desc = e.code === "network" ? `cannot reach server \`${host}\`: ${e.message}`
        : e.code === "timeout" ? `server \`${host}\` did not respond in time: ${e.message}`
        : e.code === "non-ok-status" ? `server \`${host}\` returned ${e.message}`
