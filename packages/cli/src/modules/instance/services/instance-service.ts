...
export interface InstanceService {
  list(): Promise<
    Result<readonly Instance[], TransportError | AuthRequiredError>
  >;
  get(
    id: string,
  ): Promise<Result<Instance | null, TransportError | AuthRequiredError>>;
  deleteAgent(
    agentId: string,
...
export function createInstanceService(deps: {
  ...
  return {
    async list() {
      return trpcCall(
        () => deps.trpc.instances.list.query() as Promise<readonly Instance[]>,
      );
    },
    async get(id) {
      try {
        return ok((await deps.trpc.instances.get.query({ id })) as Instance);
      } catch (e) {
