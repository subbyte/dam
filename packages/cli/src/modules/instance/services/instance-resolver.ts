...
import type { AmbiguousError, AuthRequiredError, NotFoundError, TransportError } from "../domain/errors.js";
import type { InstanceService } from "./instance-service.js";

export const INSTANCE_ID_PREFIX = "inst-";

export type ResolveError = NotFoundError | AmbiguousError | TransportError | AuthRequiredError;

export interface InstanceResolver {
  resolve(ref: string): Promise<Result<Instance, ResolveError>>;
}

export function createInstanceResolver(deps: { instanceService: InstanceService }): InstanceResolver {
  return {
    async resolve(ref) {
